...
public class FilePathUtils {
     ...
     * @Description
     *      拼接当前文件绝对路径
     *      注：user.dir不同环境路径不同，使用前需修改后续拼接部分(eg：[项目文件夹]{0，1}/src/main/java)
    ...
    public static String getSourceFolderAbsolutePath(String path, String afterUserDirPath) {
        return StringUtils.isBlank(path)
                ? ( System.getProperty("user.dir")+afterUserDirPath )
                : path;
    }
    ...
    /**
     * @Description
     *      拼接当前文件绝对路径（本项目MybatisPlusMysqlGenerator适用）
     * @Return java.lang.String
     * @Author _lizy
     * @Date 2020/10/19 2:05
     */
    public static String getMySrcFolderAbsolutePath(String path) {
        return getSourceFolderAbsolutePath(path, "\\dubbo-provider\\src\\main\\java");
    }
    ...
    static class StringBuilderChain  {
        ...
        public StringBuilderChain append(String str) {
            Optional.ofNullable(stringBuilder).orElse(new StringBuilder(str)).append(str); // stringBuilder是否为空，为空新建，否则，拼接字符串
            return this;
        }
